...
async def put_usuario(usuario_id : int,
                      ...
                      usuario:UsuarioSchemaUp = Depends(get_usuario)):
    async with db as session:
        query = select(UsuarioModel).filter(UsuarioModel.id == usuario_id)
        result = await session.execute(query)
        usuario_up: UsuarioShemaUp = result.scalars().unique().one_or_none()
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_session)):
        ...
        return JSONResponse(content={"acces_token": criar_token_acesso(sub=usuario.id),
                                     "token_type": "bearer"},
